...
void CentSchedRouter::initialize() {
    ...
    predictor = PredictorFactory::generatePredictor(predictorType);
    predictor->initialize(this);
    ...
    for (int i = 0; i < numPorts; ++i) {
        ...
        if (g->isConnectedOutside()) {
            if (i == corePort) {
                p = new DoubleBufferFlatPort(this, g, ports, numVCs,
                        pipelineDepth, flitsPerVC);
            } else {
...
int CentSchedRouter::analyzeMeshTopology() {
        ...
        if (neighbour->getModuleType() == cModuleType::get(routerType)) {
            ...
            if ((rx == x) && (ry == y)) {
                throw cRuntimeError(
                        ...
                        getFullPath().c_str(), neighbour->getFullPath().c_str(),
                        x, y);
            } else if ((rx == x) && (ry == y + 1)) {
                ...
                if (southPort != -1) {
                    ...
                    EV << "-I- " << getFullPath() << " connected through out["
                              << op << "] to South port: "
                              << neighbour->getFullPath() << endl;
                    southPort = op;
                }
            } else if ((rx == x) && (ry == y - 1)) {
                ...
                if (northPort != -1) {
                    throw cRuntimeError(
                            "Already found a north port: %d for ports: %s."
                                    " %s is miss-configured", northPort,
                            getFullPath().c_str(),
                            neighbour->getFullPath().c_str());
                } else {
                    EV << "-I- " << getFullPath() << " connected through out["
                ...
                if (westPort != -1) {
                    throw cRuntimeError(
                            "Already found a west port: %d for ports: %s."
                                    " %s is miss-configured", westPort,
                            getFullPath().c_str(),
                            neighbour->getFullPath().c_str());
                } else {
                    EV << "-I- " << getFullPath() << " connected through out["
                ...
                if (eastPort != -1) {
                    throw cRuntimeError(
                            "Already found an east port: %d for ports: %s."
                                    " %s is miss-configured", eastPort,
                            getFullPath().c_str(),
                            neighbour->getFullPath().c_str());
                } else {
                    EV << "-I- " << getFullPath() << " connected through out["
                ...
                throw cRuntimeError(
                        "Found a non Mesh connection between %s (%d,%d) and %s (%d,%d)",
                        " %s is miss-configured", getFullPath().c_str(), rx, ry,
            ...
            if (corePort >= 0) {
                throw cRuntimeError(
                        "Already found core on port: %d but now on %d",
                        corePort, op);
            }
            corePort = op;
        } else {
            throw cRuntimeError(
                    "Port: %d is connected to module %s of unknown type %s (not %s or %s)",
                    op, neighbour->getFullName(), neighbour->getClassName(),
                    routerType, coreType);
        }
    }
    ...
    if (corePort < 0) {
        EV << "-W- " << getFullPath()
                  << " could not find corePort (of coreType:" << coreType << ")"
                  << endl;
    }
    return (0);
}
...
void CentSchedRouter::handlePop(NoCPopMsg* msg) {
    ...
    // Debug code which shows time since last head every 10000 ticks
//    if (ticks%10000 == 0) {
//        for(int i=0; i < numPorts; ++i)
//            if(ports[i] != NULL)
//                (ports[i])->showDiffs();
...
void CentSchedRouter::handleMessage(cMessage *msg) {
    ...
    case NOC_FLIT_MSG:
        ...
        break;
    case NOC_POP_MSG:
        handlePop((NoCPopMsg*) msg);
        break;
    case NOC_CREDIT_MSG:
        handleCredit((NoCCreditMsg*) msg);
        break;
    default:
...
void CentSchedRouter::callPredictor(NoCFlitMsg* msg, FlatPortIfc* inPort,
        FlatPortIfc* outPort, vc_t& inVC) {

    predictor->predict(msg, inPort, outPort, inVC);
}
